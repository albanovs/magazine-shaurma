...
export default function Basket() {
    ...
    const removeSpice = (itemIndex, spiceIndex) => {
        const updatedCart = [...cartItems];
        if (updatedCart[itemIndex].spices) {
            updatedCart[itemIndex].spices.splice(spiceIndex, 1);
    ...
    const calculateTotal = () => {
        return cartItems.reduce((total, item, index) => {
            ...
            const spicesPrice =
                (item.spices?.reduce((sum, spice) => sum + spice.price, 0) || 0) * quantities[index];
            return total + basePrice + spicesPrice;
        }, 0);
    };
        ...
        <div className="text-white mb-20">
            ...
            <div className="flex lg:flex-row flex-col gap-10">
                                        ...
                                        <ul className="text-sm text-gray-300 mt-2 space-y-1">
                                            {item.spices.map((spice, i) => (
                                                <li key={i} className="flex items-center">
                                                    <span>+ {spice.name} ({spice.price} ₽)</span>
                                                    <button
                                                        onClick={() => removeSpice(index, i)}
                            ...
                            <div className="text-lg flex gap-2 font-semibold mt-4 lg:mt-0">
                                <h6>
                                    {(
                                        ...
                                        (item.spices?.reduce((sum, s) => sum + s.price, 0) || 0)
                                    ).toLocaleString()}
                                </h6> <h6>₽</h6>
                            </div>
                        </div>
                ...
                <div className="lg:w-[30%] p-5 min-h-[250px] h-full bg-[#2B445D] rounded-[20px]">
                    ...
                    <a
                        href="/order"
                        className="w-full block text-center text-[#0F1F2F] font-semibold cursor-pointer px-5 mt-10 bg-yellow-400 hover:bg-yellow-500 py-3 rounded-lg"
                    >
